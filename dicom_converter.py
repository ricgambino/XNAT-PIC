...
class Bruker2DicomConverter():
    ...
    def __init__(self, folder_to_convert, dst_folder):
    ...
    def multi_processes_conversion(self, params):
    ...
    def convert(self, dirs):
        ...
        if 'Results' in dirs[0].split('/')[-1]:
            rel_path = os.path.relpath(convert_path, self.folder_to_convert)
            dst_path = os.path.join(dst_path, 'MR', 'resources', rel_path).replace('\\', '/')
