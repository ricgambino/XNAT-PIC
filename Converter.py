...
thread_semaphore = threading.BoundedSemaphore(3)

class DICOMConverter(Thread):

    def __init__(self, name, folder_to_convert, dst_folder, master):
    ...
    def run(self):
        ...
        print('Starting ' + str(self.folder_to_convert.split('/')[-1]) + ' conversion')
        ...
        list_path = sorted(os.listdir(self.folder_to_convert))
        # Check for 'results' folder into list_path
        if hasattr(self.master, 'results_flag') is False:
            self.master.results_flag = 0
        ...
        for i, path in enumerate(list_path, 1):
            ...
            if 'Results' not in path:
                current_path = os.path.join(self.folder_to_convert, path).replace('\\', '/')
                if os.path.isdir(current_path):
                    ...
                    if dseq_file != []: 
                        ...
                        rel_path = os.path.relpath(res, self.folder_to_convert)
                        # parent_folder = os.path.basename(head2)
                        dst_path = os.path.join(self.dst_folder, 'MR', rel_path).replace('\\', '/')
                    ...
                    if 'Custom' in current_path:
                        shutil.copy(current_path, self.dst_folder)
            else:
                if self.master.results_flag == 1:
                    current_path = os.path.join(self.folder_to_convert, path).replace('\\', '/')
                    res = current_path
                    rel_path = os.path.relpath(res, self.folder_to_convert)
                    dst_path = os.path.join(self.dst_folder, 'MR', 'resources', rel_path).replace('\\', '/')
                    shutil.copytree(current_path, dst_path)
            # if self.master.conv_flag == 1:
        ...
        if parameters:
            ...
            self.master.root.config(cursor="arrow") 
            self.master.convert_btn['state'] = tk.NORMAL
            self.master.info_btn['state'] = tk.NORMAL
            self.master.upload_btn['state'] = tk.NORMAL
            self.master.process_btn['state'] = tk.NORMAL
            # sys.exit(1)
            # Inserire folder nella lista delle directory non convertite

        print('Ending ' + str(self.folder_to_convert.split('/')[-1]) + ' conversion')  
        end_time = time.time()
